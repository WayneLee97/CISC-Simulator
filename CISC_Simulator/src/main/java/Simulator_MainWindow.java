...
public class Simulator_MainWindow extends javax.swing.JFrame
{
    ...
    public void cycle()
    {
        ...
        {
            R0_Field.setText(registers.getR0().getData());
            R1_Field.setText(registers.getR1().getData());
            R2_Field.setText(registers.getR2().getData());
            R3_Field.setText(registers.getR3().getData());
            
            X1_Field.setText(registers.getX1().getData());
            X2_Field.setText(registers.getX2().getData());
            X3_Field.setText(registers.getX3().getData());
            
            PC_Field.setText(registers.getPC().getData());
            //CC_Field.setText(registers.getCC());
            IR_Field.setText(registers.getIR().getData());
            MBR_Field.setText(registers.getMBR().getData());
            MAR_Field.setText(registers.getMAR().getData());
            MFR_Field.setText(registers.getMFR().getData());
            
            
            Value_Field.setText(memory.getMemory(instructions.binary_to_int(Address_Field.getText())).getData());
            
        }
        
        //implement the instruction for this step
        if(!this.memory.getMemory(this.instructions.binary_to_int(registers.getPC().getData())).equals("0000000000000000")){
			//execute the instruction,but first use the translator function to get the operation code
			int op_code = op_translator(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData(), instructions);
			switch (op_code) {
			//load register from the memory, LDR
			case 1:
				registers.setIR(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData());
				this.instructions.LDR(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData(), registers, this.memory);
				break;
			//store register from memory, STR
			case 2:
				registers.setIR(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData());
				this.instructions.STR(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData(), registers, this.memory);
				break;
			//load register with address, LDA
			case 3:
				registers.setIR(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData());
				this.instructions.LDA(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData(), registers, this.memory);
				break;
			//load index register from the memory, LDX
			case 41:
				registers.setIR(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData());
				this.instructions.LDX(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData(), registers, this.memory);
				break;
			//store index register to memory, STX
			case 42:
				registers.setIR(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData());
				this.instructions.STX(memory.getMemory(instructions.binary_to_int(registers.getPC().getData())).getData(), registers, this.memory);
				break;
			default:
				break;
			}
		...
		int next_address = this.instructions.binary_to_int(registers.getPC().getData()) + 1;
